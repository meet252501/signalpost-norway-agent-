...
from norway_company_agent.external_control import (
    development_score,
    run_company_control,
    strategy_order,
)
...
from norway_company_agent.identity import (
...
from norway_company_agent.refresh import diff_datasets, diff_profile
from norway_company_agent.research import (
    answer_profile,
    parse_screen_query,
    screen_profiles,
)
from norway_company_agent.sampling import (
    deterministic_extension_sample,
    deterministic_financial_filer_sample,
    deterministic_website_audit_sample,
    financial_filer_eligible,
    normalize_row,
    stratum,
)
...
from norway_company_agent.workspace import (
    load_workspace,
    record_screen,
    save_workspace,
)
from scripts.build_prototype import compact as compact_prototype
from scripts.build_prototype import qualification_copy
from scripts.build_verified_observations import (
    build as build_verified_observations,
)
from scripts.discover_linkedin_company_profiles import (
...
from scripts.extract_company_site_activity import (
    observation as site_activity_observation,
)
from scripts.extract_company_site_news import (
    observation as site_news_observation,
)
from scripts.normalize_google_maps_results import candidate_score
from scripts.run_annual_report_workforce_connector import (
    extract_candidate,
    needs_ocr,
)
from scripts.run_brave_discovery import brave_search
from scripts.run_fagfolkguiden_reviews_connector import (
...
from scripts.run_linkedin_guest_jobs_connector import (
    canonical_company_url,
    parse_detail_company_urls,
    parse_job_cards,
    parse_typeahead,
)
from scripts.run_scrapy_websites import terminal_events_for_run
from scripts.run_sentiment_model import (
    MODEL_REVISION,
    normalize_generated_label,
)
from scripts.score_company_completeness import score_rows, summarize
...
class EvidenceTests(unittest.TestCase):
    ...
    def test_not_fetched_is_distinct_from_not_applicable(self):
        record = evidence(
            ...
            "official",
            "https://example.test",
            note="No filing flag in snapshot",
        )
...
class ExternalFootprintTests(unittest.TestCase):
    ...
    def test_linkedin_guest_jobs_require_exact_verified_company_url(self):
        ...
        jobs, candidates = parse_job_cards(
            raw, "https://linkedin.com/company/af-gruppen"
        )
        self.assertEqual(candidates, 2)
        self.assertEqual([item["job_id"] for item in jobs], ["4456746433"])
        self.assertEqual(
            jobs[0]["company_url"], "https://linkedin.com/company/af-gruppen"
        )

    def test_linkedin_company_urls_and_typeahead_are_normalized_without_claiming_ambiguous_ids(
        self,
    ):
        self.assertEqual(
            canonical_company_url(
                "https://no.linkedin.com/company/Norsk-Fiskeeksport/about?trk=x"
            ),
            ...
            json.dumps(
                [
                    {"id": "34440", "type": "COMPANY", "displayName": "AF Gruppen"},
                    {
                        "id": "1188022",
                        "type": "COMPANY",
                        "displayName": "AF Gruppen Sverige",
                    },
                ]
            ).encode(),
            "AF GRUPPEN ASA",
    ...
    def test_linkedin_guest_profile_uses_structured_company_data_and_ignores_dormant_challenge_code(
        self,
    ):
        graph = {
            "@graph": [
    ...
    def test_linkedin_guest_profile_rejects_authwall_without_organization_data(self):
        with self.assertRaisesRegex(RuntimeError, "no structured organization"):
            extract_linkedin_profile(
                b"<script>recaptcha/challengepage</script>",
                "https://linkedin.com/company/example",
            )

    def test_linkedin_stale_handle_fallback_is_bounded_to_registry_legal_name(self):
        self.assertEqual(
            legal_name_profile_url("DIPS AS"),
            "https://www.linkedin.com/company/dips-as",
        )
        self.assertEqual(
            legal_name_profile_url("RØD & BLÅ AS"),
            "https://www.linkedin.com/company/rod-bla-as",
        )
    ...
    def test_linkedin_discovery_requires_exact_typeahead_name_and_corroboration(self):
        ...
        self.assertEqual(
            [
                ...
                for item in parse_linkedin_exact_typeahead(raw, "DIPS AS")
            ],
            ["1"],
        )
        ...
        company = {
            ...
            "evidence": {
                "website": {
                    "status": "available",
                    "value": {"final_url": "https://dips.com"},
                }
            },
        }
        ...
        weak = linkedin_discovery_identity(
            company,
            {
                "name": "DIPS AS",
    ...
    def test_linkedin_fuzzy_discovery_uses_verified_site_alias_and_reverse_domain(self):
        company = {
            ...
            "evidence": {
                "website": {
                    "status": "available",
                    "value": {"final_url": "https://jarre.co", "title": "Jarre&Co"},
                },
                "roles": {
                    "value": {
                        "roles": [{"name": "Christian Jarre", "role_code": "DAGL"}]
                    }
                },
            },
        }
    ...
    def test_linkedin_profile_identity_accepts_redirect_alias_only_with_name_or_reverse_domain_proof(
        self,
    ):
        profile = {
            "name": "ZAPTEC ASA",
            ...
            "evidence": {
                "website": {
                    "source_url": "https://www.zaptec.com/",
                    "value": {"final_url": "https://www.zaptec.com/"},
                }
            },
        }
        accepted = assess_linkedin_profile_identity(
            ...
            {
                "name": "Zaptec",
                "page_url": "https://linkedin.com/company/zaptec",
                "website": "https://www.zaptec.com",
            },
        )
        ...
        rejected = assess_linkedin_profile_identity(
            ...
            "https://linkedin.com/company/gozaptec",
            {
                ...
                "page_url": "https://linkedin.com/company/unrelated",
                "website": "https://parent.test",
            },
        )
        self.assertFalse(rejected["publishable_candidate"])

    def test_google_play_observation_is_supported_but_unofficial_output_stays_experimental(
        self,
    ):
        item = self.observation(
    ...
    def test_company_directory_is_not_a_website_discovery_candidate(self):
        profile = {
            "name": "OBLOMOV AS",
            "organisation_number": "991167315",
            "municipality": "SOLA",
        }
        result = {
            "url": "https://www.northdata.com/Oblomov-AS/BR-991167315",
            "title": "Oblomov AS",
            "snippet": "991167315",
            "rank": 1,
        }
    ...
    def test_unknown_company_directory_with_org_number_is_not_a_candidate(self):
        profile = {
            "name": "AKSLA AS",
            "organisation_number": "923304290",
            "municipality": "ÅLESUND",
        }
        result = {
            "url": "https://vexter.no/selskap/aksla-as/923304290",
            "title": "AKSLA AS",
            "snippet": "923304290",
            "rank": 1,
        }
        self.assertFalse(
            score_search_candidate(profile, result)["publishable_candidate"]
        )
    ...
    def test_annual_workforce_parser_does_not_treat_norwegian_o_as_zero(self):
        ...
        self.assertEqual(
            extract_candidate(heading), (None, None, "no_employee_phrase", None)
        )
        ...
        self.assertEqual(
            extract_candidate("Selskapet har 1 2025 sysselsatt 2 arsverk.")[0], 2
        )
        self.assertEqual(
            extract_candidate("Antall arsverk syssetsatt i regnskapsaret: 3")[0], 3
        )
        self.assertEqual(
            extract_candidate("Stiftelsen har ingen ansatte og ingen arsverk.")[0], 0
        )
        self.assertEqual(
            extract_candidate("Selskapet hadde ingen ansatte i 2025.")[0], 0
        )
        self.assertEqual(
            extract_candidate("Gjennomsnittlig antall ansatte i regnskapsaret: 0")[0], 0
        )
        self.assertEqual(
            extract_candidate("Note Antall Aarsverk i regnskapsaret 0.00")[0], 0
        )
        self.assertEqual(
            extract_candidate("Tal pa Aarsverk i rekneskapsaret 1.50")[0], 1.5
        )
        self.assertTrue(needs_ocr("Digital cover text without the employee note"))
        self.assertFalse(
            needs_ocr("Selskapet har 2 ansatte. " + "Digital report text. " * 8)
        )

    def test_aggregate_keeps_source_metrics_separate_and_abstains_on_thin_sentiment(
        self,
    ):
        items = [
            self.observation(
                id="a", sentiment_label="positive", sentiment_model_version="m1"
            ),
            self.observation(
                id="b",
                platform="youtube",
    ...
    def test_customer_review_sentiment_accepts_ten_independent_reviewers_on_one_platform(
        self,
    ):
        items = [
            self.observation(
    ...
    def test_google_maps_identity_gate_rejects_neighbor_and_accepts_exact_address(self):
        ...
        exact = candidate_score(
            profile,
            {
                "title": "AF Gruppen",
                "address": "Standardveien 1, 0581 Oslo, Norge",
                "phone": "+47 22 89 11 00",
                "web_site": "https://afgruppen.no/",
                "review_count": 21,
            },
        )
        neighbor = candidate_score(
            profile,
            {
                ...
                "address": "Standardveien 1, 0581 Oslo, Norge",
                "phone": "+47 22 89 11 00",
                "web_site": "https://afgruppen.no/eiendom/",
                "review_count": 0,
            },
        )
    ...
    def test_google_maps_exact_name_and_postcode_city_can_resolve_operating_address(
        self,
    ):
        profile = {
        ...
        result = candidate_score(
            profile,
            {
                "title": "Example Industri AS",
                "address": "Factory Road 7, 4021 Stavanger, Norway",
                "phone": "",
                "web_site": "",
                "review_count": 4,
            },
    ...
    def test_google_maps_trade_name_requires_exact_address_phone_and_no_partial_name_collision(
        self,
    ):
        profile = {
            ...
            "evidence": {
                "registry": {
                    "value": {
                        "forretningsadresse.adresse": "Stranden 81",
                        "forretningsadresse.postnummer": "0250",
        ...
        candidate = {
            "title": "PrivatMegleren Premium",
            "address": "Stranden 81, 0250 Oslo",
            "phone": "+47 22 62 00 00",
        }
    ...
    def test_controller_recomputes_sentiment_and_records_marginal_gain(self):
        profile = {
            ...
            "name": "Example AS",
            "evidence": {
                "website": {"value": {"identity_assessment": {"publishable": True}}}
            },
        }
        handle = self.observation(
        ...
        result = run_company_control(
            profile, [handle], minimum_iterations=10, maximum_iterations=15
        )
        self.assertGreaterEqual(result["iterations_run"], 10)
        self.assertTrue(any(item["score_delta"] > 0 for item in result["iterations"]))
        self.assertTrue(
            all("sentiment_status" in item for item in result["iterations"])
        )
    ...
    def test_controller_final_score_is_not_path_dependent_after_target_is_reached(self):
        ...
        result = run_company_control(
            profile, observations, target=20, minimum_iterations=1
        )
...
class CompletenessScoreTests(unittest.TestCase):
    def test_all_source_weights_sum_to_one_hundred(self):
        from scripts.score_company_completeness import (
            ENRICHMENT_WEIGHTS,
            FOUNDATION_WEIGHTS,
        )

        self.assertEqual(
            sum(FOUNDATION_WEIGHTS.values()) + sum(ENRICHMENT_WEIGHTS.values()), 100.0
        )

    def test_all_source_score_combines_foundation_and_external_without_imputing_missing(
        self,
    ):
        profile = {
            "organisation_number": "923609016",
            "evidence": {
                "registry_live": {
                    "status": "available",
                    "value": {"organisation_number": "923609016"},
                },
        ...
        result = {
            ...
            "final": {"components": components, "experimental_components": components},
        }
    ...
    def test_site_activity_requires_exact_identity_and_preserves_snapshot_provenance(
        self,
    ):
        profile = {
            "organisation_number": "923609016",
        ...
        profile["evidence"]["website"]["value"]["identity_assessment"][
            "publishable"
        ] = False
    ...
    def test_news_title_gate_requires_the_full_legal_name_core(self):
        self.assertTrue(
            exact_title_match(
                "NORDIC DOOR AS", "Nordic Door AS åpner ny fabrikk - Lokalavisa"
            )
        )
        self.assertFalse(
            exact_title_match(
                "NORDIC DOOR AS", "Nordic investors prefer another door - Example"
            )
        )
        self.assertTrue(
            exact_title_match(
                "SOLVANG ASA", "Sterkt årsresultat fra Solvang ASA i 2024 - Skipsrevyen"
            )
        )
        self.assertFalse(
            exact_title_match(
                "VIND HOLDING AS",
                "Inntektene til Aneo Roan Vind Holding AS stupte - mn24.no",
            )
        )
        self.assertFalse(
            exact_title_match("CONSTO AS", "Drastisk fall hos Consto Bergen AS - BT")
        )
    ...
    def test_site_news_requires_exact_identity_and_a_captured_news_path(self):
            ...
            "evidence": {
                "website": {
                    ...
                    "value": {
                        ...
                        "pages": [
                            {
                                "url": "https://example.test/aktuelt/new-contract",
                                "title": "New contract",
                                "content_sha256": "a" * 64,
                            }
                        ],
        ...
        self.assertTrue(publishable_observation(item))
        profile["evidence"]["website"]["value"]["pages"][0]["url"] = (
    ...
    def test_verified_observations_require_known_org_and_snapshot_hash(self):
        ...
        seed = {
            ...
            "content_sha256": "a" * 64,
            "evidence_span": "Example AS",
            "proof": "Exact legal name",
        }
        self.assertTrue(
            build_verified_observations([seed], profiles)[0]["exact_entity"]
        )
    ...
    def test_directory_identity_is_experimental_and_never_becomes_strict(self):
        item = {
            ...
            "content_sha256": "a" * 64,
            "exact_entity": True,
            "identity_proof": [{"type": "organisation_number", "value": "923609016"}],
            "acquisition_mode": "rights_review_experiment",
            "rights_status": "review_required",
            "source_class": "public_company_directory",
            "strategy": "company_directory_identity",
        }
        profile = {
            "organisation_number": "923609016",
            "name": "Example AS",
        ...
        self.assertEqual(
            score["experimental_components"]["exact_external_identity"], 20.0
        )
        self.assertFalse(publishable_observation(item))
...
class SamplingTests(unittest.TestCase):
    def test_financial_filer_sample_requires_current_active_rows_and_preserves_overlap(
        self,
    ):
        fields = [
            "organisasjonsnummer",
        ...
        rows = [
            {
                "organisasjonsnummer": str(200000000 + index),
                "navn": f"Company {index}",
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            selected, metadata = deterministic_financial_filer_sample(
                path,
                5,
                latest_year="2025",
                preserved_organisation_numbers={"200000003"},
                seed=7,
            )
            repeated, _ = deterministic_financial_filer_sample(
                path,
                5,
                latest_year="2025",
                preserved_organisation_numbers={"200000003"},
        ...
        self.assertEqual(
            [row["organisation_number"] for row in selected],
            [row["organisation_number"] for row in repeated],
        )
    ...
    def test_extension_sample_is_deterministic_and_excludes_initial(self):
        ...
        rows = [
            {
                "organisasjonsnummer": str(100000000 + index),
                "navn": f"Company {index}",
                "hjemmeside": "",
                "organisasjonsform.kode": "AS",
            }
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with gzip.open(path, "wt", encoding="utf-8", newline="") as handle:
                ...
                writer.writeheader()
                writer.writerows(rows)
            first, metadata = deterministic_extension_sample(
                path, 5, {"100000000", "100000001"}, seed=3
            )
            second, _ = deterministic_extension_sample(
                path, 5, {"100000000", "100000001"}, seed=3
            )
        self.assertEqual(
            [item["organisation_number"] for item in first],
            [item["organisation_number"] for item in second],
        )
    ...
    def test_strata_distinguish_adverse_and_web_coverage(self):
        ...
        self.assertEqual(
            stratum({**base, "bankrupt": True, "website": ""}), "AS|5-19|adverse|no-web"
        )
        ...
        rows = [
            {
                ...
                "navn": "Excluded AS",
                "hjemmeside": "excluded.no",
                "organisasjonsform.kode": "AS",
            },
            {
                "organisasjonsnummer": "222222222",
                "navn": "A AS",
                "hjemmeside": "https://www.shared.no/a",
                "organisasjonsform.kode": "AS",
            },
            {
                "organisasjonsnummer": "333333333",
                "navn": "B AS",
                "hjemmeside": "shared.no/b",
                "organisasjonsform.kode": "AS",
            },
            {
                "organisasjonsnummer": "444444444",
                "navn": "C AS",
                "hjemmeside": "unique.no",
                "organisasjonsform.kode": "AS",
            },
        ]
        with tempfile.TemporaryDirectory() as directory:
            ...
            with gzip.open(path, "wt", encoding="utf-8", newline="") as handle:
                ...
                writer.writeheader()
                writer.writerows(rows)
            selected, metadata = deterministic_website_audit_sample(
                path, 1, {"111111111"}, {"shared.no"}, seed=9
            )
...
class OperationsTests(unittest.TestCase):
    def test_history_rate_limiter_spaces_request_starts_not_responses(self):
        from norway_company_agent import official
    ...
    def test_unknown_evidence_state_is_submission_error(self):
        self.assertEqual(
            evidence_terminal_state({"status": "not_fetched"}), "submission_error"
        )
    ...
    def test_nearest_rank_percentiles_are_deterministic(self):
        ...
        self.assertEqual(
            latency_summary([1, 2, 3]),
            {"n": 3, "p50_ms": 2.0, "p95_ms": 3.0, "max_ms": 3.0},
        )

    def test_domain_fairness_summary_preserves_tail(self):
        result = domain_request_summary(
            __import__("collections").Counter({"a.no": 1, "b.no": 2, "c.no": 9})
        )
...
class WebsiteTests(unittest.TestCase):
    ...
    def test_crawl_event_extraction_and_merge_preserve_page_hashes(self):
        ...
        record = merge_profile_events(
            {"website": "https://example.no"}, [homepage, secondary]
        )
        self.assertEqual(record["status"], "available")
    ...
    def test_only_extracts_declared_social_links(self):
        soup = BeautifulSoup(
            '<a href="https://www.linkedin.com/company/example/">LinkedIn</a><a href="/about">About</a>',
            "html.parser",
        )
        self.assertEqual(
    ...
    def test_social_profiles_canonicalize_www_variants(self):
        self.assertEqual(
            normalize_social_url("https://www.facebook.com/Example/"),
            {"platform": "facebook", "url": "https://facebook.com/Example"},
        )
        self.assertEqual(
            normalize_social_url(
                "https://linkedin.com/company/example/admin/feed/posts"
            ),
            {"platform": "linkedin", "url": "https://linkedin.com/company/example"},
        )
        self.assertEqual(
            normalize_social_url("https://youtube.com/channel/abc/featured"),
            {"platform": "youtube", "url": "https://youtube.com/channel/abc"},
        )
    ...
    def test_priority_pages_stay_on_exact_site(self):
        soup = BeautifulSoup(
            '<a href="/kontakt">Contact</a><a href="https://other.no/about">About</a><a href="/products">Products</a>',
            "html.parser",
        )
        self.assertEqual(
            _priority_links("https://example.no/", soup), ["https://example.no/kontakt"]
        )
...
class DiscoveryTests(unittest.TestCase):
    ...
    def test_brave_parser_is_provider_neutral_candidate_input(self):
        results = parse_brave_web_results(
            {
                "web": {
                    "results": [
                        {
    ...
    def test_directory_and_social_results_are_not_company_site_candidates(self):
        profile = {
            "organisation_number": "923609016",
            "name": "Example Norge AS",
            "municipality": "OSLO",
        }
        for url in (
            "https://proff.no/selskap/example",
            "https://linkedin.com/company/example",
        ):
            with self.subTest(url=url):
                self.assertEqual(
                    score_search_candidate(
                        profile, {"url": url, "title": "Example Norge AS"}
                    )["status"],
                    "rejected",
                )
    ...
    def test_exact_name_in_title_and_host_is_only_a_crawl_candidate(self):
        ...
        decision = choose_search_candidate(
            profile,
            [
                {
    ...
    def test_ambiguous_name_match_without_host_support_abstains(self):
        ...
        decision = choose_search_candidate(
            ...
            [
                {
                    "url": "https://parent-group.no/",
                    "title": "Norsk Fiskeeksport AS - portfolio",
                    "snippet": "Group companies",
                }
            ],
        )
...
class SentimentTests(unittest.TestCase):
    ...
    def item(item_id, label="positive", source="https://news.example/a", **changes):
        item = {
            ...
            "source_url": source,
            "retrieved_at": "2026-08-22T00:00:00Z",
            "evidence_span": "Exact-company event sentence.",
            "content_sha256": "a" * 64,
        }
        item.update(changes)
        return item
    ...
    def test_company_owned_or_unhashed_items_are_not_publishable(self):
        self.assertFalse(
            publishable_sentiment_item(self.item(1, source_class="company_owned"))
        )
    ...
    def test_inference_input_requires_exact_entity_independent_source_and_hash(self):
        ...
        accepted, reasons = sentiment_input_eligibility(
            {**item, "exact_entity": False, "content_sha256": None}
        )
    ...
    def test_rollup_requires_two_independent_publishers(self):
        ...
        self.assertEqual(
            aggregate_company_sentiment(same_publisher)["status"], "abstain"
        )
    ...
    def test_perfect_balanced_300_item_corpus_passes_poc_gate(self):
        ...
        predictions = [
            self.item(i, labels[i % 4], source=f"https://news{i % 7}.example/item/{i}")
            for i in range(300)
        ]
    ...
    def test_wrong_entity_and_company_owned_predictions_fail_gate(self):
        ...
        predictions = [
            self.item(i, labels[i % 3], source=f"https://news.example/{i}")
            for i in range(300)
        ]
    ...
    def test_qualification_minimum_cannot_be_weakened(self):
        with self.assertRaises(ValueError):
            evaluate_predictions(
                [{"id": "1", "label": "positive"}], [self.item(1)], minimum_items=1
            )


class OfficialNormalizationTests(unittest.TestCase):
    def test_accounting_obligation_is_categorical_for_as_but_not_enk(self):
        company = accounting_obligation_assessment(
            {"organisation_number": "923609016", "legal_form": "AS"}
        )
        sole_trader = accounting_obligation_assessment(
            {"organisation_number": "923609017", "legal_form": "ENK", "employees": 0}
        )
        self.assertEqual(company["value"]["classification"], "required_by_legal_form")
        self.assertEqual(
            sole_trader["value"]["classification"], "threshold_or_activity_dependent"
        )
    ...
    def test_observed_filing_overrides_rule_path(self):
        record = accounting_obligation_assessment(
            {
                "organisation_number": "923609018",
                "legal_form": "ENK",
        ...
        body = [
            {
                ...
                "resultatregnskapResultat": {
                    "driftsresultat": {
                        "driftsresultat": 0,
                        "driftsinntekter": {"sumDriftsinntekter": 12},
                    }
                },
            }
        ]
    ...
    def test_financial_history_is_sorted_and_links_to_official_pdfs(self):
        record = normalize_financial_history(
            ["2024", "2022", "2024", "invalid"], "923609016"
        )
            ...
            "rollegrupper": [
                {
                    ...
                    "roller": [
                        {
                            "type": {"kode": "LEDE", "beskrivelse": "Chair"},
                            "person": {
                                "fodselsdato": "1970-01-01",
                                "navn": {"fornavn": "Ada", "etternavn": "Nord"},
                            },
                        }
                    ],
                }
...
class ResearchAgentTests(unittest.TestCase):
        ...
        return {
            ...
            "evidence": {
                ...
                "financials": evidence(
                    ...
                    value={"records": [{"revenue": revenue, "annual_result": 1}]},
                    content_sha256="b" * 64,
                ),
            },
        }
    ...
    def test_cross_company_screen_has_exact_membership_and_inspectable_plan(self):
        ...
        result = screen_profiles(
            rows,
            "companies in Oslo with more than 10 employees and revenue over 1 million",
        )
        self.assertFalse(result["abstained"])
        self.assertEqual(
            [item["organisation_number"] for item in result["results"]], ["111111111"]
        )
        self.assertEqual(len(result["plan"]["filters"]), 3)
        self.assertTrue(
            all(
                citation["content_sha256"]
                for citation in result["results"][0]["citations"]
            )
        )
    ...
    def test_unsupported_cross_company_criterion_abstains(self):
        ...
        result = screen_profiles(
            [], "companies in Oslo with positive Glassdoor sentiment"
        )
    ...
    def test_workspace_saves_pins_history_and_recovers_corruption(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            updated = record_screen(
                workspace,
                {
                    "query": "in Oslo",
                    "plan": {"filters": []},
                    "result_count": 1,
                    "results": [{"organisation_number": "111111111"}],
                },
                pin_organisations=["111111111"],
            )
    ...
    def test_sentiment_abstains_and_every_fact_has_a_source(self):
        row = {
            ...
            "evidence": {
                "registry": evidence(
                    "registry", "available", "official", "https://example.test"
                )
            },
        }
        ...
        self.assertTrue(
            any(
                "Sentiment is not scored" in item
                for item in result["unsupported_or_uncertain"]
            )
        )
        self.assertTrue(all(fact["source_url"] for fact in result["facts"]))
        self.assertFalse(
            any(fact["claim"] == "Registry employee count" for fact in result["facts"])
        )
    ...
    def test_natural_leads_word_routes_to_roles(self):
        roles = evidence(
            ...
            value={"roles": [{"name": "Ada Nord", "role": "Chair", "inactive": False}]},
        )
        row = {
            "organisation_number": "923609016",
            "name": "Example AS",
    ...
    def test_quarantined_website_claims_are_not_returned(self):
        website = evidence(
            ...
            value={
                ...
                "social_links": [
                    {
                        "platform": "linkedin",
                        "url": "https://linkedin.com/company/parent",
                    }
                ],
        ...
        self.assertTrue(
            any("quarantined" in item for item in result["unsupported_or_uncertain"])
        )
...
class RefreshTests(unittest.TestCase):
    ...
    def test_missing_to_zero_is_a_real_change_with_provenance(self):
        source = evidence(
            "registry", "available", "official", "https://example.test/entity"
        )
        old = {
            "organisation_number": "923609016",
            "employees": None,
...
class WebsiteIdentityTests(unittest.TestCase):
    def test_group_contact_page_listing_subsidiary_org_number_is_not_exact_homepage_identity(
        self,
    ):
        profile = {
                ...
                "website": evidence(
                    ...
                    value={
                        ...
                        "pages": [
                            {
                                "title": "Contact",
                                "main_text_excerpt": "SKS Produksjon AS organisation number 915 637 353",
                            }
                        ],
                    },
                )
    ...
    def test_shared_identity_gate_quarantines_parent_social_links(self):
        website = evidence(
            ...
            value={
                "title": "Parent Group",
                "main_text_excerpt": "Parent Group portfolio",
                "social_links": [
                    {
                        "platform": "linkedin",
                        "url": "https://linkedin.com/company/parent",
    ...
    def test_exact_legal_name_is_publishable(self):
        row = {
            ...
            "evidence": {
                "website": {
                    "status": "available",
                    "value": {"title": "Norsk Fiskeeksport AS"},
                }
            },
        }
    ...
    def test_parent_brand_without_legal_name_is_quarantined(self):
        row = {
            "organisation_number": "988412406",
            "name": "Tevlingveien 23 Invest AS",
            "evidence": {
                "website": {"status": "available", "value": {"title": "Ragde Eiendom"}}
            },
        }
    ...
    def test_parked_domain_and_parent_company_sports_site_are_quarantined(self):
        parked = {
            "organisation_number": "996081001",
            "name": "Condalign AS",
            "evidence": {
                "website": {
                    "status": "available",
                    "value": {"title": "CondAlign.com is for sale | HugeDomains"},
                }
            },
        }
        sports = {
            "organisation_number": "996242692",
            "name": "Primulator B.I.L.",
            "evidence": {
                "website": {
                    "status": "available",
                    "value": {
                        "title": "Primulator",
                        "description": "Premium products for HoReCa",
                    },
                }
            },
        }
    ...
    def test_hosting_placeholder_and_generic_link_page_are_quarantined(self):
        hosting = {
            "organisation_number": "917568278",
            "name": "HJELMEN AS",
            "evidence": {
                "website": {
                    "status": "available",
                    "value": {
                        "title": "www.Hjelmen-as.no is parked at Miss Hosting Web Hosting",
                        "main_text_excerpt": "Hjelmen " * 100,
                    },
                }
            },
        }
        links = {
            "organisation_number": "986606009",
            "name": "KOALA ANS",
            "evidence": {
                "website": {
                    "status": "available",
                    "value": {
                        "title": "koala.no",
                        "description": "Find the best information and most relevant links on all topics related to",
                    },
                }
            },
        }
    ...
    def test_social_handle_requires_exact_entity_name_evidence(self):
        ...
        self.assertFalse(
            assess_social_identity(
                aon, {"platform": "linkedin", "url": "https://linkedin.com/company/aon"}
            )["publishable"]
        )
        self.assertTrue(
            assess_social_identity(
                ...
                {
                    "platform": "linkedin",
                    "url": "https://linkedin.com/company/norsk-fiskeeksport",
                },
            )["publishable"]
        )
...
class VerifiedSiteSeedTests(unittest.TestCase):
    def test_verified_seed_is_applied_and_unknown_org_is_rejected(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            profiles.write_text(
                json.dumps({"organisation_number": "123456789", "name": "Example AS"})
                + "\n"
            )
            seeds.write_text(
                json.dumps(
            ...
            command = [
                sys.executable,
                str(ROOT / "scripts" / "apply_verified_site_seeds.py"),
                "--profiles",
                str(profiles),
                "--seeds",
                str(seeds),
