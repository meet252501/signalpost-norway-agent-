...
def norm(value: object) -> str:
    ...
    return " ".join(
        token for token in re.findall(r"[a-z0-9]+", text) if token not in LEGAL
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Find exact-name public YouTube channels with bounded yt-dlp search."
    )
    ...
    for profile in read_jsonl(Path(args.profiles)):
        ...
        if website.get("status") != "available" or not (
            value.get("identity_assessment") or {}
        ).get("publishable"):
            continue
    ...
    options = {
        ...
        "no_warnings": True,
        "extract_flat": True,
        "playlistend": args.search_results,
        "skip_download": True,
    }
    for profile in candidates[: args.limit]:
        ...
        site_domain = domain(
            website.get("source_url") or (website.get("value") or {}).get("final_url")
        )
        statuses["searched"] += 1
        try:
            ...
            digest = hashlib.sha256(
                json.dumps(
                    matched, ensure_ascii=False, sort_keys=True, default=str
                ).encode()
            ).hexdigest()
            proof = [
                {
                    "type": "company_site_identity",
                    "score": (
                        (website.get("value") or {}).get("identity_assessment") or {}
                    ).get("score"),
                },
                {
                    "type": "exact_normalized_youtube_channel_name",
                    "value": core,
                    "matched_search_results": len(matched),
                },
            ]
            common = {
            ...
            observations.append(
                {
                    **common,
                    "id": f"youtube-search-handle-{org}-{digest[:16]}",
                ...
                observations.append(
                    {
                        **common,
                        ...
                        "metrics": {
                            ...
                            "recent_views_mean": round(sum(views) / len(views), 1),
                        },
    ...
    report = {
        "connector": "yt_dlp_exact_channel_search_v1",
        "eligible": len(candidates),
        **statuses,
        "observations": len(observations),
        "errors": errors,
        "claim_boundary": "Exact-name, repeated-result YouTube channel candidates; experimental and terms-sensitive.",
    }
