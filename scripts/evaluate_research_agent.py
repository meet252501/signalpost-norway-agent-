...
def main() -> None:
    ...
    single = answer_profile(
        by_org[single_spec["organisation_number"]], single_spec["question"]
    )
    ...
    for spec in suite["screens"]:
        ...
        citations_complete = all(
            ...
            and citation.get("retrieved_at")
            and citation.get("content_sha256")
        ...
        screen_results.append(
            {
                "query": spec["query"],
                "expected": spec["expected_organisation_numbers"],
                "actual": actual,
                "exact_membership_and_order": exact,
    ...
    saved_work = (
        ...
        and len(workspace["history"]) == len(suite["screens"])
    ...
    report = {
        ...
        "maximum": 12,
        "qualification_passed": single_supported
        and screen_rate == 1
        and plan_rate == 1
        and abstention
        and saved_work
    ...
    print(
        json.dumps(
            {key: value for key, value in report.items() if key != "screen_results"},
            ensure_ascii=False,
            indent=2,
        )
    )
