...
def _tokens(value: Any) -> list[str]:
    ...
    return [
        ...
        if token not in LEGAL_AND_GENERIC and len(token) > 1
    ]
...
def assess_website_identity(profile: dict[str, Any]) -> dict[str, Any]:
    ...
    hostname = (
        urllib.parse.urlparse(
            value.get("final_url") or website.get("source_url") or ""
        ).hostname
        or ""
    )
    ...
    normalized_raw = (
        unicodedata.normalize("NFKD", candidate_text)
        .encode("ascii", "ignore")
        .decode()
        .casefold()
    )
    homepage_token_sets = [
        set(_tokens(part)) for part in homepage_identity_parts if part
    ]
    exact_homepage_name = bool(
        core and any(set(core).issubset(tokens) for tokens in homepage_token_sets)
    )
    substantive_homepage = len(str(value.get("main_text_excerpt") or "").strip()) >= 100
    is_business_sports_club = bool(
        re.search(
            r"(?:^|\s)B\.?\s*I\.?\s*L\.?(?:\s|$)",
    ...
    if any(marker in normalized_raw for marker in parked_markers):
        score = 0.1
        reasons.append(
            "captured page is a parked, for-sale, or generic hosting placeholder"
        )
    elif (
        is_business_sports_club
        and "bedriftsidrett" not in normalized_candidate_text
        and "b i l" not in normalized_candidate_text
    ):
        score = 0.3
        reasons.append(
            "business sports-club entity points to the operating company's site without club evidence"
        )
    elif org_digits and org_digits in compact_homepage_candidate:
        score = 1.0
        reasons.append(
            "exact organisation number appears in homepage identity evidence"
        )
    elif len(core) >= 2 and exact_homepage_name:
        score = 0.95
        reasons.append(
            "all normalized legal-name tokens appear together in homepage identity evidence"
        )
    elif len(core) == 1 and exact_homepage_name and substantive_homepage:
        score = 0.95
        reasons.append(
            "single distinctive legal-name token appears in homepage identity evidence with substantive content"
        )
    elif ratio >= 0.75 and len(overlap) >= 2:
        score = 0.85
        reasons.append(
            "most legal-name tokens appear, but exact identity is incomplete"
        )
        ...
        title_tokens = _tokens(value.get("title") or "")
        fuzzy_title_score = rapidfuzz.fuzz.token_sort_ratio(
            " ".join(core), " ".join(title_tokens)
        )
        desc_tokens = _tokens(value.get("description") or "")
        fuzzy_desc_score = rapidfuzz.fuzz.token_sort_ratio(
            " ".join(core), " ".join(desc_tokens)
        )
        best_fuzzy = max(fuzzy_title_score, fuzzy_desc_score)

        if best_fuzzy >= 92:
            score = 0.92
            reasons.append(
                f"high fuzzy match score ({best_fuzzy:.1f}) in title/description"
            )
        elif best_fuzzy >= 75 and len(overlap) >= 1:
            score = 0.85
            reasons.append(
                f"moderate fuzzy match score ({best_fuzzy:.1f}) with some token overlap"
            )
        else:
            score = 0.3
            reasons.append(
                "registry-linked URL lacks strong exact-entity identity evidence"
            )
    status = (
        "exact"
        if score >= 0.9
...
def assess_social_identity(
    profile: dict[str, Any], link: dict[str, str]
) -> dict[str, Any]:
    core = _tokens(profile.get("name"))
