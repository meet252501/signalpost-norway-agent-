...
def assess_website_identity(profile: dict[str, Any]) -> dict[str, Any]:
    ...
    if any(marker in normalized_raw for marker in parked_markers):
        ...
        fuzzy_title_score = rapidfuzz.fuzz.token_sort_ratio(" ".join(core), " ".join(title_tokens))
        desc_tokens = _tokens(value.get("description") or "")
        fuzzy_desc_score = rapidfuzz.fuzz.token_sort_ratio(" ".join(core), " ".join(desc_tokens))
        best_fuzzy = max(fuzzy_title_score, fuzzy_desc_score)
        
        if best_fuzzy >= 92:
            score = 0.92
            reasons.append(f"high fuzzy match score ({best_fuzzy:.1f}) in title/description")
        elif best_fuzzy >= 75 and len(overlap) >= 1:
            score = 0.85
            reasons.append(f"moderate fuzzy match score ({best_fuzzy:.1f}) with some token overlap")
        else:
            score = 0.3
