...
def build_company_search_query(profile: dict[str, Any]) -> str:
    ...
    if not name or not org:
        raise ValueError(
            "Company discovery requires a legal name and organisation number"
        )
...
def score_search_candidate(
    ...
    if not normalized:
        return {
            ...
            "reasons": ["invalid HTTP(S) candidate URL"],
        }
    host = (
    ...
    if any(
        ...
        return {
            ...
            "host": host,
            "reasons": [
                "directory, aggregator, or social host is not a company website candidate"
            ],
        }

    name_tokens = [
        token
        for token in _tokens(profile.get("name"))
    ...
    org = re.sub(r"\D", "", str(profile.get("organisation_number") or ""))
    evidence_digits = re.sub(
        r"\D", "", f"{result.get('title', '')} {result.get('snippet', '')}"
    )
    ...
    all_name_tokens_in_title = bool(
        name_tokens and set(name_tokens).issubset(set(title_tokens))
    )
    name_in_host = bool(name_compact and name_compact in host_compact)
    municipality_match = bool(
        municipality_tokens and municipality_tokens <= set(snippet_tokens)
    )
    score = 0.0
    reasons = []
    if org_match:
    ...
    if all_name_tokens_in_title:
        ...
        score += 0.25
        reasons.append(
            "all distinctive legal-name tokens appear across result evidence"
        )
    if name_in_host:
        score += 0.3
    ...
    publishable_candidate = (
        score >= 0.75 and name_in_host and (org_match or all_name_tokens_in_title)
    )
    return {
        "status": "accepted_for_crawl"
        if publishable_candidate
        else "review"
        if score >= 0.6
        else "rejected",
        "score": score,
