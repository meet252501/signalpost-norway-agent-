...
def _claim(
    label: str, value: Any, record: dict[str, Any], classification: str
) -> dict[str, Any]:
    return {
        "claim": label,
        "value": value,
...
def answer_profile(row: dict[str, Any], question: str) -> dict[str, Any]:
        ...
        if records:
            ...
            for label, key in (
                ...
                if latest.get(key) is not None:
                    facts.append(
                        _claim(label, latest[key], financial, "official_annual_account")
                    )
        else:
            unsupported.append(
    ...
    if all_topics or any(term in q for term in ("lead", "role")):
        people = [
            item
            for item in (roles.get("value") or {}).get("roles", [])
            if not item.get("inactive")
        ]
        for person in people[:12]:
            facts.append(
                _claim(
                    person.get("role") or person.get("group") or "Registered role",
                    person.get("name") or person.get("organisation_number"),
                    roles,
    ...
    if all_topics or any(term in q for term in ("location", "where")):
        ...
        if not items:
            unsupported.append(
                "No registered subunit was returned; this does not prove the company has no physical presence."
            )

    website = evidence.get("website", {})
    if all_topics or "social" in q:
        value = website.get("value") or {}
        website_publishable = (value.get("identity_assessment") or {}).get(
            "publishable", True
        )
        ...
        for item in (value.get("social_links") or []) if website_publishable else []:
            facts.append(
                _claim(
                    f"Declared {item['platform']} profile",
                    item["url"],
                    website,
        ...
        if website.get("status") == "available" and not website_publishable:
            unsupported.append(
                "A registry-linked website was fetched, but exact legal-entity identity was not established; its claims and social links are quarantined."
            )
        if website.get("status") != "available":
            unsupported.append(
                "The registry-linked company website was not available to this run."
            )
...
def _latest_financial(row: dict[str, Any]) -> dict[str, Any]:
    records = (row.get("evidence", {}).get("financials", {}).get("value") or {}).get(
        "records"
    ) or []
...
def parse_screen_query(query: str) -> dict[str, Any]:
    ...
    unsupported = [
        message for term, message in UNSUPPORTED_SCREEN_TERMS.items() if term in lower
    ]

    municipality = re.search(
        r"\b(?:in|municipality(?:\s+is|\s*=)?)\s+([a-zæøåéü .'-]+?)(?=\s+(?:with|and|having|that|where)\b|$)",
        lower,
    )
    if municipality:
        filters.append(
    ...
    industry = re.search(
        r"\bindustry(?:\s+contains|\s+is|\s*=)?\s+[\"']([^\"']+)[\"']",
        text,
        flags=re.IGNORECASE,
    )
    if industry:
...
def _screen_value(row: dict[str, Any], field: str) -> Any:
    ...
    if field == "website":
        ...
        return record.get("status") == "available" and bool(
            (record.get("value") or {})
            .get("identity_assessment", {})
            .get("publishable", True)
        )
    if field == "financials":
        return (
...
def screen_profiles(rows: list[dict[str, Any]], query: str) -> dict[str, Any]:
    ...
    if not plan["executable"]:
        return {
            ...
            or "No supported criterion was recognized.",
        }
    results = []
    for row in rows:
        if not all(
            _compare(_screen_value(row, item["field"]), item["operator"], item["value"])
        ...
        if plan.get("sort"):
            evidence_modules.add(
                "financials" if plan["sort"]["field"] == "revenue" else "registry"
            )
