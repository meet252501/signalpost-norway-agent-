...
def development_score(
    ...
    identity_ok = bool(
        (website.get("value") or {}).get("identity_assessment", {}).get("publishable")
    ) or any(
        (
            item.get("platform") == "wikidata"
            and item.get("signal_type") == "company_profile"
        )
        or (
            item.get("platform") == "google_places"
            and item.get("signal_type") == "place_summary"
        )
        for item in accepted
    )
    sentiment = aggregate_footprint(accepted).get("sentiment", {})
    experimental_sentiment = [
        item for item in experimental if item.get("sentiment_label")
    ]
    experimental_sentiment_hosts = {
        (urlparse(str(item.get("source_url"))).hostname or "")
        .casefold()
        .removeprefix("www.")
    ...
    experimental_sentiment_ready = (
        bool(experimental_review_summaries)
        or len(experimental_sentiment) >= 10
        and (
            len(experimental_sentiment_hosts) >= 2
            or len(experimental_sentiment_reviewers) >= 10
        )
    )
    pieces = {
        ...
        "workforce_jobs": 10.0
        if signals & {"job_posting", "workforce_snapshot"}
        else 0.0,
        "public_buzz": 10.0
        if signals & {"public_post", "public_mention", "buzz_metrics"}
        else 0.0,
        ...
        if accepted
        and all(
            item.get("retrieved_at") and item.get("content_sha256") for item in accepted
        )
        else 0.0,
    }
    potential_signals = signals | {
        str(item.get("signal_type")) for item in experimental
    }
    ...
    potential_identity_ok = identity_ok or any(
        ...
        or (
            item.get("platform") == "company_directory"
            and item.get("signal_type") == "company_profile"
        )
        for item in experimental
    )
    potential_pieces = {
        **pieces,
        ...
        if potential_identity_ok and (accepted or experimental)
        else 0.0,
        "verified_handles": min(15.0, len(potential_handle_platforms) * 5.0),
        "profile_metrics": 10.0
        if "profile_metrics" in potential_signals
        else pieces["profile_metrics"],
        "places_identity": 5.0
        if "place_summary" in potential_signals
        else pieces["places_identity"],
        "places_reviews": 10.0
        if potential_signals & {"review", "review_summary"}
        else pieces["places_reviews"],
        "workforce_jobs": 10.0
        if potential_signals & {"job_posting", "workforce_snapshot"}
        else pieces["workforce_jobs"],
        "public_buzz": 10.0
        if potential_signals & {"public_post", "public_mention", "buzz_metrics"}
        else pieces["public_buzz"],
        "independent_sentiment": 15.0
        if experimental_sentiment_ready
        else pieces["independent_sentiment"],
    }
    return {
        ...
        "experimental_sentiment_status": "available"
        if experimental_sentiment_ready
        else "abstain",
...
def strategy_order(
    prior_iterations: list[dict[str, Any]], strategies: tuple[str, ...] = STRATEGIES
) -> list[str]:
    ...
    for item in prior_iterations:
        gains[str(item["strategy"])].append(
            float(item.get("learning_gain") or item.get("score_delta") or 0)
        )
...
def strategy_history(
    prior_iterations: list[dict[str, Any]], strategy: str
) -> dict[str, Any]:
    ...
    return {
        ...
        "prior_mean_learning_gain": round(sum(rewards) / len(rewards), 3)
        if rewards
        else 0.0,
    }
...
def run_company_control(
    ...
    *,
    ...
    iteration_budget = (
        ...
        else min(maximum_iterations, len(remaining))
    )
    for iteration in range(1, iteration_budget + 1):
        ...
        experimental_delta = round(
            current["experimental_potential_score"]
            - previous["experimental_potential_score"],
            3,
        )
        ...
        iterations.append(
            {
                "iteration": iteration,
                "strategy": strategy,
                "controller_action": "replicate"
                if history["prior_mean_learning_gain"] > 0
                else "explore",
                **history,
                ...
                "experimental_sentiment_status": current[
                    "experimental_sentiment_status"
                ],
                "result": (
                    "reinforce_publishable"
                    if delta > 0
                    else "reinforce_experimental"
                    if experimental_delta > 0
                    else "hold"
                    if additions
