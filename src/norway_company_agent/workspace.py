...
def load_workspace(path: str | Path) -> tuple[dict[str, Any], str | None]:
    ...
    try:
        ...
        return (
            empty_workspace(),
            f"Workspace recovery reset invalid state: {type(exc).__name__}",
        )
...
def record_screen(
    ...
    result: dict[str, Any],
    *,
    pin_organisations: list[str] | None = None,
) -> dict[str, Any]:
    ...
    updated.setdefault("history", []).append(
        {
            ...
            "result_organisation_numbers": [
                item.get("organisation_number") for item in result.get("results", [])
            ],
        }
    )
